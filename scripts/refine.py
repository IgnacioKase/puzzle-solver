...
feat = features(cv.GaussianBlur(ref, (0, 0), 1.2))
...
sifts = {p["id"]: p for p in json.loads((OUT / "matches.json").read_text())}
...
def solve(p):
    ...
    candidates = p["dense"][:]
    sm = sifts[p["id"]]["match"]
    if sm and sm["inliers"] >= 3 and 0.52 < sm["scale"] < 0.64:
        candidates.append({**sm, "score": 0})
    distinct = []
    for c in candidates:
        if not any(
            np.linalg.norm(np.array(c["center"]) - d["center"]) < 35
            and abs((c["angle"] - d["angle"] + 180) % 360 - 180) < 20
            for d in distinct
        ):
            distinct.append(c)
    refined = []
    for cand in distinct[:5]:
        ...
        x0 = max(0, int(x) - 180)
        y0 = max(0, int(y) - 180)
        x1 = min(1800, int(x) + 180)
        y1 = min(2600, int(y) + 180)
        roi = feat[y0:y1, x0:x1]
        best = None
        for angle in np.arange(cand["angle"] - 6, cand["angle"] + 6.1, 2):
            for scale in [0.56, 0.577, 0.595]:
                patch, pm, H = transformed(f, mask, angle, scale)
                pm = np.float32(pm > 200)
                if (
                    patch.shape[0] >= roi.shape[0]
                    or patch.shape[1] >= roi.shape[1]
                    or pm.sum() < 100
                ):
                    continue
        ...
        if best:
            HH = np.array(best["H"])
            wm = cv.warpAffine(mask, HH, (1800, 2600)) > 200
            best["gapFraction"] = float((gap[wm] > 0).mean()) if wm.sum() else 0
            best["siftInliers"] = (
                sm["inliers"]
                if sm and np.linalg.norm(np.array(sm["center"]) - best["center"]) < 18
                else 0
            )
            refined.append(best)
    ...
    if refined:
        ...
        alts = [
            c
            for c in refined[1:]
            if np.linalg.norm(np.array(c["center"]) - best["center"]) > 40
            or abs((c["angle"] - best["angle"] + 180) % 360 - 180) > 35
        ]
...
if __name__ == "__main__":
    ps = json.loads((OUT / "dense.json").read_text())
    with ThreadPoolExecutor(max_workers=4) as pool:
        res = list(pool.map(solve, ps))
    (OUT / "refined.json").write_text(json.dumps(res, indent=2))
