...
def read(name, width=None):
    ...
    if width:
        im = cv.resize(im, (width, round(im.shape[0] * width / im.shape[1])))
    return im
...
def warp(im, corners, size=(1800, 2600)):
    ...
    H = cv.getPerspectiveTransform(
        np.float32(corners), np.float32([[0, 0], [w - 1, 0], [w - 1, h - 1], [0, h - 1]])
    )
...
def prepare():
    ...
    state, _H = warp(
        read("puzzle-state.jpg", 1373), [[128, 46], [1254, 60], [1365, 1763], [27, 1780]]
    )
    # Feature registration refines the manually identified outer corners.
    sift = cv.SIFT_create(nfeatures=22000, contrastThreshold=0.02)
    ...
    HH, keep = cv.findHomography(
        np.float32([k1[m.queryIdx].pt for m in good]),
        np.float32([k2[m.trainIdx].pt for m in good]),
        cv.RANSAC,
        4,
    )
    ...
    sheets = [
        (
            ...
            "sheet-a-main.jpg",
            1373,
            [[110, 90], [1215, 87], [1225, 1665], [96, 1670]],
            (1400, 2000),
            10,
            7,
        ),
    ...
    pieces = []
    for name, source, width, corners, size, _rows, _cols in sheets:
        ...
        for x0, y0, x1, y1 in cuts[name]:
            cv.line(mask, (x0, y0), (x1, y1), 0, 4)
        ...
        for c in contours:
            ...
            if name == "A":
                row = int(
                    np.argmin(
                        abs(np.array([140, 340, 510, 700, 895, 1090, 1280, 1480, 1680, 1880]) - cy)
                    )
                )
                xs = (
                    [120, 310, 500, 690, 885, 1085, 1290]
                    if row < 5
                    else [120, 340, 520, 710, 905, 1100, 1285]
                )
                col = int(np.argmin(abs(np.array(xs) - cx)))
            elif name == "B":
                row = int(
                    np.argmin(
                        abs(np.array([120, 350, 565, 790, 1010, 1230, 1450, 1660, 1890]) - cy)
                    )
                )
                xs = [120, 315, 490, 670, 845, 1000, 1150, 1305]
                if row == 7:
                    xs = [150, 320, 510, 700, 905, 1130, 1310]
                if row == 8:
                    xs = [150, 340, 515, 710, 930, 1090, 1310]
                col = int(np.argmin(abs(np.array(xs) - cx)))
            else:
                row = 0 if cy < 260 else 1
                col = round((cx - 125) / 198)
            ...
            fullmask = np.zeros(sheet.shape[:2], np.uint8)
            cv.drawContours(fullmask, [c], -1, 255, -1)
            crop = sheet[y0:y1, x0:x1]
            pmask = fullmask[y0:y1, x0:x1]
            ...
            pieces.append(
                {
                    ...
                    "contour": (c[:, 0, :] - [x0, y0]).tolist(),
                }
            )
            cv.drawContours(annotated, [c], -1, (0, 160, 0), 2)
            cv.putText(
                annotated, pid, (x, max(18, y)), cv.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2
            )
...
def match():
    ...
    sift = cv.SIFT_create(nfeatures=35000, contrastThreshold=0.012, edgeThreshold=15)
    refs = []
    ...
    for _i, p in enumerate(pieces):
        ...
        if des is not None and len(kp) > 3:
            for name, _ref, rkp, rdes in refs:
                ...
                for ratio in [0.7, 0.8, 0.9]:
                    ...
                    dst = np.float32([rkp[m.trainIdx].pt for m in good])
                    ...
                    n = int(inliers.sum())
                    if n < 3 or not 0.25 < scale < 1.5:
                        continue
                    center = (H @ np.array([*p["center"], 1])).tolist()
                    if not (0 < center[0] < 1800 and 0 < center[1] < 2600):
                        continue
                    residual = np.linalg.norm(src @ H[:, :2].T + H[:, 2] - dst, axis=1)[
                        inliers.ravel().astype(bool)
                    ]
                    pts = src[inliers.ravel().astype(bool)]
                    spread = (
                        float(cv.contourArea(cv.convexHull(pts))) / p["area"] if len(pts) > 2 else 0
                    )
                    candidates.append(
                        {
                            "reference": name,
        ...
        candidates.sort(
            key=lambda c: c["inliers"] + min(c["spread"], 0.3) * 10 - c["error"], reverse=True
        )
        ...
        print(
            p["id"],
            ("{:.0f} matches at {:.0f},{:.0f}".format(p["match"]["inliers"], *p["match"]["center"]))
            if candidates
            else "unmatched",
            flush=True,
        )
