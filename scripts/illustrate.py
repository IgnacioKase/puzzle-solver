...
draw.text(
    (30, 20), "How a placement is checked: the small upper-left gap", font=font(27), fill="#183e35"
)
draw.text(
    (30, 64),
    "Illustrative candidates A-g5 and B-a4. Their individual physical fits have not been confirmed.",
    font=font(18),
    fill="#455b50",
)
box = (0, 245, 350, 610)
for x, title, image in [
    (30, "1. Gap in the supplied photograph", state),
...
for x, pid in [(30, ids[0]), (490, ids[1])]:
    ...
    draw.text(
        (x + 175, 745),
        f"{p['x'] / 18:.1f}% across / {p['y'] / 26:.1f}% down",
        font=font(16),
        fill="#455b50",
    )
...
draw.rectangle(
    (1165 + box[0] * sx, 645 + box[1] * sy, 1165 + box[2] * sx, 645 + box[3] * sy),
    outline="#00aa55",
    width=3,
)
...
draw.text(
    (30, 880),
    "Evidence: matching artwork, the photographed empty area, and the surrounding piece boundaries.",
    font=font(19),
    fill="#183e35",
)
draw.text(
    (30, 920),
    "The middle panel is a computed overlay, not a photograph of a completed assembly.",
    font=font(18),
    fill="#455b50",
)
...
draw.text(
    (24, 15),
    "One inventory entry, three useful representations: B-a4",
    font=font(25),
    fill="#183e35",
)
...
for x, title, graphic in [
    (30, "Color artwork", im),
    (440, "Binary mask: white = piece", Image.fromarray(alpha).convert("RGBA")),
...
draw.text(
    (30, 412),
    "Color suggests a location. The mask excludes paper. The outline supports geometric checks.",
    font=font(20),
    fill="#455b50",
)
...
for x, label, name in [
    (25, "Loose pieces on numbered sheets", "sheet-a-main.jpg"),
