...
import cv2 as cv, numpy as np, json, time, math
...
ref=cv.imread(str(O/'reference.jpg'))
ref=cv.resize(ref,(600,867),interpolation=cv.INTER_AREA)
feat=features(ref)
state=cv.resize(cv.imread(str(O/'state.jpg')),(600,867),interpolation=cv.INTER_AREA)
...
def solve(p):
    ...
    for angle in range(-180,180,10):
        ...
        if pm.sum()<25:continue
        score=cv.matchTemplate(feat,patch,cv.TM_CCOEFF_NORMED,mask=pm)
        np.nan_to_num(score,copy=False,nan=-1,posinf=-1,neginf=-1)
        cx,cy=H@np.array([*p['center'],1]);dx=int(cx);dy=int(cy)
        ok=gap[dy:dy+score.shape[0],dx:dx+score.shape[1]]
        if ok.shape!=score.shape:continue
        score[ok==0]=-1
        for _ in range(2):
            _,v,_,pos=cv.minMaxLoc(score)
            x,y=pos;HH=H.copy();HH[:,2]+=[x,y]
            candidates.append(dict(score=v,angle=angle,H=(HH*factor).tolist(),center=((HH@np.array([*p['center'],1]))*factor).tolist()))
            score[max(0,y-22):y+23,max(0,x-22):x+23]=-1
    candidates.sort(key=lambda c:c['score'],reverse=True)
    result={**p,'dense':candidates[:8]}
    print(p['id'],round(candidates[0]['score'],3) if candidates else None,'%.1fs'%(time.time()-t),flush=True)
    return result

if __name__=='__main__':
    import sys
    ps=json.loads((O/'pieces.json').read_text())
    if len(sys.argv)>1:ps=[p for p in ps if p['id'] in sys.argv[1:]]
    with ThreadPoolExecutor(max_workers=4) as pool:
        results=list(pool.map(solve,ps))
    (O/('dense-test.json' if len(sys.argv)>1 else 'dense.json')).write_text(json.dumps(results,indent=2))
