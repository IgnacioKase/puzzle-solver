...
The unmodified originals are backed up outside the repository. Repository images use descriptive names such as `sheet-a-main.jpg` and `puzzle-state.jpg`, without capture timestamps. Removing metadata does not remove information visible in a photograph. Removing color profiles or auxiliary HDR data may affect color-managed or HDR display even when the primary decoded pixel values are identical.
